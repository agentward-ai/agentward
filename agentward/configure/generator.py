...
def _apply_tool_rules(
    ...
    # Rule 1: CRITICAL risk or shell access → require approval
    if tool_perm.risk_level == RiskLevel.CRITICAL or DataAccessType.SHELL in access_types:
        require_approval.append(tool_name)

    # Rule 2: Destructive tools → require approval
    if tool_perm.is_destructive:
        require_approval.append(tool_name)

    # Rule 3: Write-capable email/messaging → restrict to read-only
    ...
    # Rule 4: Write-capable filesystem → restrict to read-only
