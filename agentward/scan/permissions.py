...
class DataAccessType(str, Enum):
    ...
    BROWSER = "browser"
    FINANCIAL = "financial"
    # Process interaction — can inject stdin into running processes.
    # Detected by ``stdin`` schema property or ``interact`` in the tool name.
    # Combined with SHELL (process-launching) this creates a REPL injection vector.
    PROCESS_STDIN = "process_stdin"
    # Runtime configuration mutation — can modify shell/interpreter paths or
    # other runtime settings that affect subsequent process invocations.
    # Detected by write-verb tool names containing "config"/"setting".
    # Combined with FILESYSTEM write access this creates a persistence vector.
...
_SCHEMA_PATTERNS: dict[str, tuple[DataAccessType, bool, bool]] = {
    ...
    # Process stdin injection — present when a tool can write to a running process's stdin.
    # ``stdin`` is the clearest signal; ``pid``/``process_id`` alone are too ambiguous
    # (used in both status-read and stdin-inject contexts).
    "stdin": (DataAccessType.PROCESS_STDIN, False, True),
    # Runtime config mutation — properties whose names directly name runtime settings.
    "default_shell": (DataAccessType.RUNTIME_CONFIG, False, True),
    "shell_path": (DataAccessType.RUNTIME_CONFIG, False, True),
    "config_key": (DataAccessType.RUNTIME_CONFIG, False, True),
    "setting_name": (DataAccessType.RUNTIME_CONFIG, False, True),
    # Browser
...
_EXECUTE_VERBS = frozenset({
    ...
    "launch", "shell", "bash", "eval", "interact",
})
...
_RESOURCE_PATTERNS: dict[str, DataAccessType] = {
    ...
    # Process stdin injection — tools named *interact* can send input to a running process.
    "interact": DataAccessType.PROCESS_STDIN,
...
def analyze_tool(tool: ToolInfo) -> ToolPermission:
    ...
    # Signal 3 (special case): Runtime config mutation detection.
    # Classify as RUNTIME_CONFIG when the tool has a WRITE/EXECUTE verb AND
    # the name contains a configuration-related keyword.  Read-only config tools
    # (get_config, list_settings, show_config) are intentionally excluded.
    _verb_early = _extract_verb(tool.name)
    _is_write_config = (
        _verb_early in (_WRITE_VERBS | _EXECUTE_VERBS)
        or tool.name.lower().startswith(("set_config", "update_config", "configure_"))
    )
    if DataAccessType.RUNTIME_CONFIG not in seen_types and _is_write_config:
        _name_lower = tool.name.lower()
        if any(kw in _name_lower for kw in ("config", "setting", "configure", "preference")):
            data_access.append(
                DataAccess(
                    type=DataAccessType.RUNTIME_CONFIG,
                    read=False,
                    write=True,
                    reason=(
                        "Tool name indicates write-capable runtime configuration "
                        "modification (shell/interpreter path reconfiguration risk)"
                    ),
                )
            )
            seen_types.add(DataAccessType.RUNTIME_CONFIG)
...
def compute_risk(
    ...
    # Process stdin injection → HIGH.
    # On its own this is HIGH (injecting arbitrary input into a running process).
    # When combined with a SHELL tool on the same server the chain detector
    # escalates this to CRITICAL via the REPL injection chain rule.
    if DataAccessType.PROCESS_STDIN in access_types:
        risk = _max_risk(risk, RiskLevel.HIGH)
        reasons.append(
            "Can inject stdin into running processes — REPL injection risk "
            "when combined with a process-launching tool"
        )

    # Runtime config write → HIGH.
    # Ability to set shell/interpreter paths enables persistent compromise
    # when combined with arbitrary filesystem write access.
    if DataAccessType.RUNTIME_CONFIG in access_types and has_writes:
        risk = _max_risk(risk, RiskLevel.HIGH)
        reasons.append(
            "Can modify runtime configuration (shell/interpreter path) — "
            "persistence risk when combined with filesystem write access"
        )

    # readOnlyHint=true on a HIGH+ risk tool is a silent-exfiltration amplifier:
    # many MCP clients auto-approve read-only tools without user confirmation,
    # meaning an attacker can silently exploit the capability.
    _risk_order = [RiskLevel.LOW, RiskLevel.MEDIUM, RiskLevel.HIGH, RiskLevel.CRITICAL]
    if (
        annotations is not None
        and annotations.read_only_hint is True
        and _risk_order.index(risk) >= _risk_order.index(RiskLevel.HIGH)
    ):
        reasons.append(
            "readOnlyHint=true on HIGH+ risk tool — MCP clients may auto-approve "
            "without user confirmation (silent exfiltration amplifier)"
        )
